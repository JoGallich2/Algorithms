...
public class RedBlackTree<T extends Comparable<T>> implements Tree<T>, Serializable {
    ...
    /*
    private void replaceChild(Node parent, Node childToReplace, Node newChild) {
        if (parent == null) {
            root = newChild;
        } else if (parent.left == childToReplace) {
    ...
    */
    ...
    private void redBlackifyDelete(Node node) {
        ...
        //Cases 3 and 4, Sibling is black with two black children.
        if(!sibling.left.isRed && !sibling.right.isRed) {
            ...
            //Case 4 parent is black
                ...
                redBlackifyDelete(node.parent);
            }
        }
        //Case 5 and 6, Black sibling with at least one red child.
        else {
            //handleBlackSiblingWithAtLeastOneRedChild(node, sibling);
            boolean nodeIsLeftChild = node == node.parent.left;

            //Case 5: Black sibling has at least one red child and the outer nephew is black.
            ...
            else if(nodeIsLeftChild && sibling.right.isRed) {
                sibling.right.isRed = false;
                sibling.isRed = true;
                rotateLeft(sibling);
                sibling = node.parent.left;
            ...
            //Case 6: black sibling with at least one red child and the outer nephew is red
            sibling.isRed = node.parent.isRed;
            node.parent.isRed = false;
            if(nodeIsLeftChild) {
                sibling.right.isRed = false;
